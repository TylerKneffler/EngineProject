    ...
    const std::string& projectName)
{
    ...
    std::error_code engineRootError;
    const fs::path resolvedEngineRoot = fs::weakly_canonical(
        fs::path(ENGINE_ROOT_PATH), engineRootError);
    std::string engineRoot = (engineRootError
        ? fs::absolute(fs::path(ENGINE_ROOT_PATH))
        : resolvedEngineRoot).generic_string();
