...
namespace
{
bool DeserializeExpandedObject(
    ...
    const SceneObjectBehavior::DeserializeOptions& options);

Engine::Core::Object* DeserializeObjectTreeInternal(
    Engine::Scene::Scene& scene,
    const JsonValue& node,
    Engine::Graphics::IGraphicsProvider* graphicsProvider,
    const std::unordered_map<std::string, SceneSerializer::Factory>& registry,
    const SceneObjectBehavior::DeserializeOptions& options)
{
    ...
    if (node.Has("prefab"))
    {
        ...
        SceneObjectBehavior::DeserializeTransform(object->transform, node["transform"]);
        if (node.Has("enabled"))
            object->enabled = node["enabled"].AsBool();
        if (node.Has("prefabOverrides"))
            SceneObjectBehavior::ApplyPrefabPatches(
                *object, node["prefabOverrides"], graphicsProvider);
        SceneObjectBehavior::ApplyBakedMaterialOverrides(
            *object, node, graphicsProvider);
        return object;
    }

    Engine::Core::Object* object = scene.AddObject();
    if (!DeserializeExpandedObject(
        *object, scene, node, graphicsProvider, registry, options))
    {
...
bool DeserializeExpandedObject(
    ...
    const SceneObjectBehavior::DeserializeOptions& options)
{
    if (!node.IsObject() || node.Has("prefab"))
        return false;

    if (node.Has("name"))
        object.name = node["name"].AsString();
    else if (options.assignDefaultNameIfMissing)
        object.name = "Object";
    ...
    for (std::size_t i = 0; i < components.ArraySize(); ++i)
    {
        ...
        const SceneSerializer::Factory* factory =
            ComponentRegistryBehavior::FindFactory(registry, type);
        if (!factory)
        {
            if (options.allowLegacyModelBindings)
            {
                if (type == "ModelNode" && options.legacyModelBindings)
                {
                    options.legacyModelBindings->emplace_back(
                        &object,
                        static_cast<unsigned>(componentNode["nodeIndex"].AsInt()));
                    continue;
                }
                if (type == "MorphTargets")
                {
                    if (Engine::Components::Mesh* mesh =
            ...
            std::string message =
                "[SceneSerializer] Unknown component type '" +
                type + "' on object '" + object.name + "' (skipped)\n";
    ...
    {
        Engine::Core::Object* child = DeserializeObjectTreeInternal(
            scene, children.ArrayAt(i), graphicsProvider, registry, options);
        if (!child)
            throw std::runtime_error("Could not instantiate child object");
        child->Parent = &object;
...
Engine::Core::Object* SceneObjectBehavior::DeserializeObjectTree(
    Engine::Scene::Scene& scene,
    const JsonValue& node,
    Engine::Graphics::IGraphicsProvider* graphicsProvider,
    const std::unordered_map<std::string, SceneSerializer::Factory>& registry,
...
{
    return DeserializeObjectTreeInternal(
        scene, node, graphicsProvider, registry, options);
}

bool SceneObjectBehavior::DeserializeObjectTreeInto(
    Engine::Core::Object& object,
    Engine::Scene::Scene& scene,
    const JsonValue& node,
    Engine::Graphics::IGraphicsProvider* graphicsProvider,
    ...
    const DeserializeOptions& options)
{
    return DeserializeExpandedObject(
        object, scene, node, graphicsProvider, registry, options);
}
