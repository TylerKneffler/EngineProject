...
bool CheckVk(VkResult result, const char* msg)
{
    ...
    {
        std::string out = std::string("[VulkanGameRenderer] ") + msg + "\n";
...
bool VulkanGameRenderer::Init(void* hwnd, uint32_t width, uint32_t height)
{
    ...
    OutputDebugStringA("[VulkanGameRenderer] Vulkan SDK support is not enabled in this build\n");
    return false;
#else
    if (!hwnd)
        return false;
    ...
    appInfo.pApplicationName = "EngineProject";
    ...
    // 3) Select physical device + queue families.
    ...
    // 7) Command buffers + sync primitives.
...
void VulkanGameRenderer::BeginFrame()
{
    ...
    {
        OutputDebugStringA("[VulkanGameRenderer] Failed to acquire next swapchain image\n");
        return;
    }
...
void VulkanGameRenderer::EndFrame()
{
    ...
    if (m_device != VK_NULL_HANDLE && m_swapchain != VK_NULL_HANDLE)
    {
        ...
        if (vkQueueSubmit(m_graphicsQueue, 1, &submitInfo, m_inFlightFences[m_currentFrame]) != VK_SUCCESS)
            OutputDebugStringA("[VulkanGameRenderer] Queue submit failed\n");
        ...
        if (presentResult != VK_SUCCESS && presentResult != VK_SUBOPTIMAL_KHR && presentResult != VK_ERROR_OUT_OF_DATE_KHR)
            OutputDebugStringA("[VulkanGameRenderer] Queue present failed\n");
    }
#endif
